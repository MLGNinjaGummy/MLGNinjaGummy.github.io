...
const ct = {
     ...
     */
    version: '1.7.0',
    meta: [{"name":"PirateShip","author":"","site":"","version":"0.0.0"}][0],
    main: {
        fpstick: 0,
        pi: 0
    },
...
ct.inputs.addAction('MoveY', [{"code":"keyboard.KeyW","multiplier":-1},{"code":"keyboard.KeyS","multiplier":1}]);
...
ct.rooms.templates['GameRoom'] = {
    name: 'GameRoom',
    width: 800,
    height: 600,
    /* JSON.parse allows for a much faster loading of big objects */
    objects: JSON.parse('[{"x":399,"y":537,"exts":{},"tx":0.75,"ty":0.75,"type":"PirateShip"}]'),
    bgs: JSON.parse('[{"depth":-1,"texture":"OceanBackground2","extends":{}}]'),
    tiles: JSON.parse('[{"depth":-10,"tiles":[],"extends":{}}]'),
    backgroundColor: '#000000',
    cameraConstraints: {"x1":800,"y1":600,"x2":800,"y2":600},
    onStep() {
        this.WhirlpoolTimer -= ct.delta;
if (this.WhirlpoolTimer <= 0 && this.WhirlpoolAmt < 10) {
    this.WhirlpoolTimer = ct.random.range(50, 150);
    this.WhirlpoolAmt++;
    this.Stage = 1;
    //console.log(this.WhirlpoolAmt + "Stage1");
    ct.types.copy(ct.random.dice("Whirlpool"), ct.random(ct.camera.width), -100);
}
else if (this.WhirlpoolTimer <= 0 && this.WhirlpoolAmt < 20) {
    this.WhirlpoolTimer = ct.random.range(40, 140);
    this.WhirlpoolAmt++;
    this.Stage = 2;
    //console.log(this.WhirlpoolAmt + "Stage2");
    ct.types.copy(ct.random.dice("Whirlpool"), ct.random(ct.camera.width), -100);
}
else if (this.WhirlpoolTimer <= 0 && this.WhirlpoolAmt < 30) {
    this.WhirlpoolTimer = ct.random.range(30, 130);
    this.WhirlpoolAmt++;
    this.Stage = 3;
    //console.log(this.WhirlpoolAmt + "Stage3");
    ct.types.copy(ct.random.dice("Whirlpool"), ct.random(ct.camera.width), -100);
}
else if (this.WhirlpoolTimer <= 0 && this.WhirlpoolAmt < 40) {
    this.WhirlpoolTimer = ct.random.range(20, 120);
    this.WhirlpoolAmt++;
    this.Stage = 4;
    //console.log(this.WhirlpoolAmt + "Stage4");
    ct.types.copy(ct.random.dice("Whirlpool"), ct.random(ct.camera.width), -100);
}
else if (this.WhirlpoolTimer<= 0) {
    this.WhirlpoolTimer = ct.random.range(0, 100);
    this.WhirlpoolAmt++;
    this.Stage = 5;
    //console.log(this.WhirlpoolAmt + "Stage5");
    ct.types.copy(ct.random.dice("Whirlpool"), ct.random(ct.camera.width), -100);
}
this.TreasureChest -= ct.delta;
    if (this.TreasureChest <= 0 && this.Stage == 1) {
        this.TreasureChest = 125;
        console.log("Stage1");
    ct.types.copy(ct.random.dice("TreasureChest"), ct.random(ct.camera.width), -100);
}
    else if (this.TreasureChest <= 0 && this.Stage == 2) {
        this.TreasureChest = 115;
        console.log("Stage2");
    ct.types.copy(ct.random.dice("TreasureChest"), ct.random(ct.camera.width), -100);
}
    else if (this.TreasureChest <= 0 && this.Stage == 3) {
        this.TreasureChest = 100;
        console.log("Stage3");
    ct.types.copy(ct.random.dice("TreasureChest"), ct.random(ct.camera.width), -100);
}
    else if (this.TreasureChest <= 0 && this.Stage == 4) {
        this.TreasureChest = 90;
        console.log("Stage4");
    ct.types.copy(ct.random.dice("TreasureChest"), ct.random(ct.camera.width), -100);
}
    else if (this.TreasureChest <= 0 && this.Stage == 5) {
        this.TreasureChest = 80;
        console.log("Stage5");
    ct.types.copy(ct.random.dice("TreasureChest"), ct.random(ct.camera.width), -100);
}
this.Mystery -= ct.delta;
if (this.Mystery <= 0) {
        this.RandomNumber = Math.floor(Math.random() * 5) + 1;
        console.log(this.RandomNumber);
        if (this.RandomNumber === 1) {
            ct.types.copy(ct.random.dice("PlusThree"), ct.random(ct.camera.width), -100);
            this.Mystery = 400;        
        }
        else if (this.RandomNumber === 2) {
            ct.types.copy(ct.random.dice("MinusFive"), ct.random(ct.camera.width), -100);
            this.Mystery = 400;        
        }
...
ct.rooms.templates['DeadRoom'] = {
    name: 'DeadRoom',
    width: 1300,
    height: 800,
    /* JSON.parse allows for a much faster loading of big objects */
    objects: JSON.parse('[{"x":600,"y":500,"exts":{},"tx":0.5,"ty":0.5,"type":"Retry"}]'),
    bgs: JSON.parse('[{"depth":0,"texture":"KrakenThingy","extends":{"repeat":"no-repeat"}}]'),
    tiles: JSON.parse('[{"depth":-10,"tiles":[],"extends":{}}]'),
    backgroundColor: '#000000',
...
this.DeadText = new PIXI.Text("U died :( Score: " + this.Score);
this.addChild(this.DeadText);
this.DeadText.x = 650;
this.DeadText.y = 400;
    },
    extends: {}
...
(function resAddon(ct) {
    ...
    ct.res = {
        ...
        soundsError: 0,
        sounds: {},
        registry: [{"Whirlpool":{"frames":1,"shape":{"type":"circle","r":115},"anchor":{"x":0.606845238095238,"y":0.513095238095238}},"OceanBackground2":{"atlas":"./img/t0.png","frames":0,"shape":{"type":"rect","top":0,"bottom":1080,"left":0,"right":1920},"anchor":{"x":0,"y":0}},"PirateShip2":{"frames":1,"shape":{"type":"rect","top":100,"bottom":70,"left":25,"right":25},"anchor":{"x":0.49615384615384617,"y":0.5214285714285715}},"TreasureChest":{"frames":1,"shape":{"type":"rect","top":250,"bottom":250,"left":250,"right":250},"anchor":{"x":0.5,"y":0.5}},"KrakenThingy":{"atlas":"./img/t1.png","frames":0,"shape":{"type":"rect","top":400,"bottom":400,"left":690,"right":690},"anchor":{"x":0.5,"y":0.5}},"Retry":{"frames":1,"shape":{"type":"rect","top":0,"bottom":525,"left":0,"right":922},"anchor":{"x":0,"y":0}},"+3":{"frames":1,"shape":{"type":"rect","top":0,"bottom":260,"left":0,"right":310},"anchor":{"x":0.3161397670549085,"y":0.18072289156626506}},"-5-removebg-preview":{"frames":1,"shape":{"type":"rect","top":190,"bottom":120,"left":230,"right":165},"anchor":{"x":0.4991023339317774,"y":0.5}}}][0],
...
ct.types.templates["PirateShip"] = {
    depth: 1,
    texture: "PirateShip2",
    onStep: function () {
        this.x+= 5.5 * ct.delta * ct.actions.MoveX.value;
if (this.x < 0) {
    this.x = 0;
}
if (this.x > 800) {
...
this.y+= 5.5 * ct.delta * ct.actions.MoveY.value;
if (this.y < 0) {
    this.y = 0;
}
if (this.y > 600) {
...
var Whirlpool = ct.place.occupied(this, this.x, this.y, "Whirlpool")
    if (Whirlpool && this.health == "alive") {
        localStorage.setItem("GameScore", ct.room.Score);
        ct.rooms.switch("DeadRoom");
    }
var TreasureChest = ct.place.occupied(this, this.x, this.y, "TreasureChest")
    if (TreasureChest && this.health == "alive") {
        ct.room.Score++;
        //console.log(ct.room.Score);
        TreasureChest.kill = true;
    }
var PlusThree = ct.place.occupied(this, this.x, this.y, "PlusThree")
    if (PlusThree && this.health == "alive") {
        ct.room.Score++;
        ct.room.Score++;
        ct.room.Score++;
        //console.log(ct.room.Score);
...
var MinusFive = ct.place.occupied(this, this.x, this.y, "MinusFive")
    if (MinusFive && this.health == "alive") {
        ct.room.Score--;
        ct.room.Score--;
        ct.room.Score--;
        ct.room.Score--;
...
ct.types.list['PirateShip'] = [];
ct.types.templates["Whirlpool"] = {
    depth: 2,
    texture: "Whirlpool",
    onStep: function () {
        ct.types.move(this);
if (this.y > 700) {
    ...
    onCreate: function () {
        this.scale.x =.5;
this.scale.y = .5;
this.speed = ct.random.range(1,5);
this.direction = ct.random.range(270-30,270+30);
this.ctype ="Whirlpool";
    },
    extends: {}
};
ct.types.list['Whirlpool'] = [];
ct.types.templates["TreasureChest"] = {
    depth: 1,
...
var DyingChest = ct.place.occupied(this, this.x, this.y, "Whirlpool")
    if (DyingChest) {
        this.kill = true;
    }
    },
...
this.speed = 1;
this.direction = ct.random.range(270-30,270+30);
this.ctype = "TreasureChest";
    },
    extends: {
    "visible": true
}
};
ct.types.list['TreasureChest'] = [];
ct.types.templates["Retry"] = {
    depth: 4,
    texture: "Retry",
    onStep: function () {
        if (ct.touch.collide(this) || (ct.mouse.hovers(this) && ct.mouse.pressed)) {
    ct.rooms.switch("GameRoom");
}
    },
    onDraw: function () {
...
ct.types.list['PlusThree'] = [];
ct.types.templates["MinusFive"] = {
    depth: 5,
    texture: "-5-removebg-preview",
    onStep: function () {
        ct.types.move(this);
if (this.y > 700) {
    this.kill = true;
